# Cloudflare Pages 正确配置

## ❌ 当前问题

您的配置中有一个错误：
- **构建命令**：`npm 安装` ❌（这是中文，应该改为英文）

## ✅ 正确的完整配置

请在 Cloudflare Dashboard 中按以下配置：

```
Framework preset: None
构建命令 (Build command): npm install --force
部署命令 (Deploy command): echo "正在通过 Cloudflare Pages 自动部署进行部署"
构建输出目录 (Build output directory): .
根目录 (Root directory): /
非生产分支部署命令 (Non-production branch deploy command): echo "Deploying preview..."
```

## 📝 详细说明

### 1. 构建命令 (Build command)
```
npm install --force
```
- **重要**：必须使用英文 `npm install`，不能使用中文 `npm 安装`
- `--force` 参数可以强制重新安装，忽略缓存问题
- **关键**：使用 `npm install` 而不是 `npm ci`，因为 `npm install` 会自动更新 `package-lock.json` 以匹配 `package.json`
- 这会安装所有依赖（phaser, wrangler 等）

### 2. 部署命令 (Deploy command)
```
echo "正在通过 Cloudflare Pages 自动部署进行部署"
```
- ✅ 这个配置是正确的
- 占位符命令，不会执行实际部署
- Cloudflare Pages 会自动部署静态文件

### 3. 构建输出目录 (Build output directory)
```
.
```
- **重要**：必须设置为 `.`（当前目录）
- 如果设置为其他目录（如 `dist`、`build`），可能找不到 `index.html`
- 所有文件都在根目录，所以输出目录就是当前目录

### 4. 根目录 (Root directory)
```
/
```
- ✅ 这个配置是正确的
- 或者可以留空（如果允许）

### 5. 非生产分支部署命令 (Non-production branch deploy command)
```
echo "Deploying preview..."
```
- 如果字段无法留空，使用这个占位符命令
- 不会执行实际部署操作

## 🔧 修复步骤

1. 登录 [Cloudflare Dashboard](https://dash.cloudflare.com/)
2. 进入 **Pages** → 你的项目 → **Settings** → **Builds & deployments**
3. 找到 "构建命令" (Build command) 字段
4. **修改为**：`npm install --force`
5. 确认 "构建输出目录" (Build output directory) 设置为 `.`
6. 确认 "非生产分支部署命令" (Non-production branch deploy command) 设置为 `echo "Deploying preview..."`
7. 保存设置
8. 触发新的部署

## ⚠️ 为什么使用 `--force`？

1. **解决缓存问题**：`--force` 可以强制重新安装，忽略缓存的旧版本
2. **版本同步**：确保安装的包版本与 `package-lock.json` 匹配
3. **避免错误**：防止 `npm ci` 因为版本不匹配而失败

## ✅ 验证配置

部署完成后，检查日志应该看到：
- ✅ `added X packages`
- ✅ `Success: Build command completed`
- ✅ `Success: Deploy command completed`
- ❌ 不应该有版本不匹配错误
- ❌ 不应该有 "npm 安装" 命令未找到的错误

## 📋 完整配置清单

请确认以下所有配置：

- [ ] Framework preset: `None`
- [ ] 构建命令: `npm install --force` ✅（必须是英文）
- [ ] 构建输出目录: `.` ✅
- [ ] 根目录: `/` ✅
- [ ] 部署命令: `echo "正在通过 Cloudflare Pages 自动部署进行部署"` ✅
- [ ] 非生产分支部署命令: `echo "Deploying preview..."` ✅

