...
export class MathCombatScene extends Scene {
    ...
    createUI() {
        ...
        const debugHandler = () => {
            console.log('=== 手动触发掉落物生成 ===');
            if (this.spawnFallingAnswer) {
                this.spawnFallingAnswer();
            } else {
                console.error('spawnFallingAnswer 方法不存在！');
            }
        };
    ...
    createVirtualButtons() {
        ...
        console.log('创建虚拟按钮 - 屏幕尺寸:', this.width, this.height, '按钮位置:', buttonSpacing, buttonY);
    ...
    update() {
        ...
        if (!this.player.body) {
            console.warn('玩家没有物理体！');
            return;
        }
        
        if (!this.player.body.enable) {
            console.warn('玩家物理体未激活！');
            return;
        }
        
        if (!this.fallingObjectsGroup) {
            console.warn('掉落物组不存在！');
            return;
        }
        ...
        fallingObjects.forEach((obj, index) => {
            ...
            if (!obj.body) {
                try {
                    ...
                    console.warn(`掉落物 ${index} 无法启用物理体:`, e);
                    return;
                }
            }
    ...
    startNewProblem() {
        console.log('=== startNewProblem 开始 ===');
        console.log('isGameOver:', this.isGameOver, 'isVictory:', this.isVictory);
        
        if (this.isGameOver || this.isVictory) {
            console.log('游戏已结束，不启动新题目');
            return;
        }
        ...
        console.log('生成题目 - 区域:', this.playerData.currentZone, '难度:', difficulty, '数学之灵:', this.currentSpirit.name);
        ...
        console.log('题目已显示:', questionText);
        ...
        console.log('创建掉落计时器 - 延迟: 800ms, 循环: true');
        this.dropTimer = this.time.addEvent({
            delay: 800, // 每0.8秒掉落一个
            callback: () => {
                console.log('计时器回调触发 - spawnFallingAnswer');
                this.spawnFallingAnswer();
            },
            loop: true
        });
        
        console.log('计时器已创建:', this.dropTimer);
        
        // 立即测试生成一个掉落物（用于调试）
        console.log('立即测试生成掉落物...');
        this.time.delayedCall(500, () => {
            console.log('延迟500ms后测试生成掉落物');
            if (this.spawnFallingAnswer) {
                this.spawnFallingAnswer();
            } else {
                console.error('spawnFallingAnswer 方法不存在！');
        ...
        console.log('=== startNewProblem 完成 ===');
    }
    
    spawnFallingAnswer() {
        console.log('=== spawnFallingAnswer 开始 ===');
        console.log('isGameOver:', this.isGameOver, 'isVictory:', this.isVictory);
        console.log('currentProblem:', this.currentProblem);
        console.log('currentSolution:', this.currentSolution);
        
        if (this.isGameOver || this.isVictory) {
            console.log('游戏已结束，不生成掉落物');
            return;
        }
        
        if (!this.currentProblem) {
            console.error('错误：currentProblem 为空！');
            return;
        }
        
        if (this.currentSolution === null || this.currentSolution === undefined) {
            console.error('错误：currentSolution 为空！');
            return;
        }
        
        // 随机掉落位置
        const x = Phaser.Math.Between(50, this.width - 50);
        console.log('掉落位置 x:', x, '屏幕宽度:', this.width);
        ...
        try {
            ...
            if (this.fallingObjectsGroup) {
                ...
                console.error('fallingObjectsGroup 不存在！');
            }
